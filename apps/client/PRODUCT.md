...
- Statically generated; no auth and no comments. Two optional Upstash-backed runtime data paths, both off unless `UPSTASH_REDIS_REST_*` is set: the activity feed (`/activity`) and the `/ama` ask box, which writes to a **private** inbox nothing renders. Every page is still prerendered from files in the repo, and nothing a visitor submits reaches a page without the author writing it up by hand.
