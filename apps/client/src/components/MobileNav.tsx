  ...
  return (
    <>
      ...
      {/* The closed panel is removed from the DOM, so its links never sit in
          the tab order behind the page. It is portaled to <body>: the sticky
          header sets `backdrop-filter`, which would otherwise make the header a
          containing block for this `position: fixed` overlay and shrink it to
          the header's box instead of the viewport. */}
      {open &&
        createPortal(
          <div
            ref={panelRef}
